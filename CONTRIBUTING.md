...
This repo packages [Fedimint Gateway](https://github.com/fedimint/fedimint) for StartOS.
...
The `gatewayd` image is built locally from `Dockerfile`, which extends the upstream `fedimint/gatewayd` image with the workarounds documented in `CLAUDE.md`. There is no `dockerTag` in the manifest.

1. Bump the `FROM fedimint/gatewayd:v<version>` line in `Dockerfile` to the new upstream tag.
2. Re-verify the Nix workarounds in `CLAUDE.md` still apply — upstream may eventually move off Nix and remove the need.
